#!/usr/bin/env bash
# Build script for ARM Mac (testing only)
# For x86_64 production builds, use build.sh on Linux

set -euo pipefail

CRDB_VERSION="${CRDB_VERSION:-v25.3.0}"
WITH_UI="${WITH_UI:-1}"
RUNTIME_BASE_IMAGE="${RUNTIME_BASE_IMAGE:-ubuntu:22.04}"

rm -rf out cockroach Dockerfile.runtime && mkdir -p out

echo "==> Building ARM64 builder image (for Mac testing)"
docker build -f Dockerfile.builder.arm64 -t crdb-builder:local .

echo "==> Cloning CockroachDB source"
git clone --depth 1 --branch "${CRDB_VERSION}" https://github.com/cockroachdb/cockroach.git

echo "==> Building CockroachDB (ARM64)"
TARGET="//pkg/cmd/cockroach:cockroach"
OUTBIN="cockroach"
if [[ "${WITH_UI}" != "1" ]]; then
  TARGET="//pkg/cmd/cockroach-short:cockroach-short"
  OUTBIN="cockroach-short"
fi

docker run --rm -t \
  -v "${PWD}/cockroach":/work/cockroach \
  -v "${PWD}/out":/work/out \
  -w /work/cockroach \
  crdb-builder:local \
  bash -lc '
    set -euo pipefail
    if [[ "'"${WITH_UI}"'" == "1" ]]; then
      bazel build //pkg/cmd/cockroach:cockroach //c-deps:libgeos
      cp -v bazel-bin/pkg/cmd/cockroach/cockroach_/cockroach /work/out/
    else
      bazel build //pkg/cmd/cockroach-short:cockroach-short //c-deps:libgeos
      cp -v bazel-bin/pkg/cmd/cockroach-short/cockroach-short_/cockroach-short /work/out/
    fi

    mkdir -p /work/out/geos
    rsync -a c-deps/libgeos/lib/ /work/out/geos/ --include="*.so*" --include="*.dylib" --exclude="*" 2>/dev/null || true
    rsync -a bazel-bin/c-deps/ /work/out/geos/ --include="*.so*" --include="*.dylib" --exclude="*" 2>/dev/null || true
    rsync -a licenses /work/out/licenses
  '

if [[ "${WITH_UI}" != "1" && -f out/cockroach-short ]]; then
  mv out/cockroach-short out/cockroach
fi

echo "==> Creating runtime Dockerfile"
cat > Dockerfile.runtime <<'EOF'
ARG RUNTIME_BASE_IMAGE
FROM ${RUNTIME_BASE_IMAGE}

RUN (command -v apt-get >/dev/null && apt-get update && apt-get install -y --no-install-recommends ca-certificates && rm -rf /var/lib/apt/lists/*) || true
RUN (command -v microdnf >/dev/null && microdnf install -y ca-certificates && microdnf clean all) || true
RUN (command -v dnf >/dev/null && dnf install -y ca-certificates && dnf clean all) || true
RUN (command -v yum >/dev/null && yum install -y ca-certificates && yum clean all) || true

RUN useradd -m -u 10001 roach
USER roach
WORKDIR /cockroach

COPY --chown=roach:roach out/cockroach /cockroach/cockroach
COPY --chown=roach:roach out/geos /cockroach/libgeos
COPY --chown=roach:roach out/licenses /cockroach/licenses

ENTRYPOINT ["/cockroach/cockroach"]
CMD ["help"]
EOF

echo "==> Building runtime image"
docker build \
  --build-arg RUNTIME_BASE_IMAGE="${RUNTIME_BASE_IMAGE}" \
  -t crdb-runtime:${CRDB_VERSION}-arm64 \
  -f Dockerfile.runtime .

echo ""
echo "✅ Build complete!"
echo "   Image: crdb-runtime:${CRDB_VERSION}-arm64"
echo "   Test: docker run --rm crdb-runtime:${CRDB_VERSION}-arm64 version"
echo ""
echo "⚠️  NOTE: This is ARM64 build for Mac testing only."
echo "   For x86_64 production build, use build.sh on Linux."
